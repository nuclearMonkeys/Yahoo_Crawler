...
class YahooSpider(scrapy.Spider):
    ...
    def parse(self, response):
        reservice = "https://answers.yahoo.com/"
        ...
        for question in response.css('div.QuestionCard__questionCard___18EIQ'):
            ...
            yield frontitem
        request = scrapy.Request(reservice, method = "PUT", headers = self.headers, callback = self.reloaded_parse)
        yield request

    def check_loadmore(self, response):
        pass

    def reloaded_parse(self, response):
        # For parsing through newly reloaded information
        frontitem = YahoocrawlerItem()
        for question in response.css('div.QuestionCard__questionCard___18EIQ'):
            #itemload = ItemLoader(frontitem = YahoocrawlerItem(), selector = question)
            #itemload.add_css('title', 'a.QuestionCard__title___1DKC-')
